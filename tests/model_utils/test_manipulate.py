...
def test_add_slot_if_not_exists_exists():
    ...
    # try to change existing has file slot to string:
    new_slot = SlotDefinition(name="has file", range="string")

    original_model = MetadataModel.init_from_path(MINIMAL_VALID_METADATA_MODEL)
...
def test_add_slot_if_not_exists_not_exists():
    ...
    assert updated_model_slots.difference(original_model_slots) == set(new_slot.name)


def test_upsert_class_slot_exists():
    """Test upsert_class_slot if the slot exists."""

    # change existing has file slot to string in the Dataset class:
    class_name = "Dataset"
    new_slot = SlotDefinition(name="has file", range="string")
